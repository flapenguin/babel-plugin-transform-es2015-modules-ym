...
module.exports = function ({ template, types: t }) {
     ...
     * foo/bar/baz/quz.esn.js   => foo.bar.baz.quz
     * foo/bar/baz/index.esn.js => foo.bar.baz
     * foo/bar/baz/Baz.esn.js   => foo.bar.Baz
     */
    ...
    function onProgramEnter(path, state) {
        ...
        state.file.importYmModule = function (name) {
            if (!state.ymImports[name]) {
                const local = name.replace(/\.([A-Z])/, (m, letter) => letter.toUpperCase());
                state.ymImports[name] = this.scope.generateUidIdentifier(local);
            }

            return state.ymImports[name];
        };
    }
        ...
        for (let statementPath of path.get('body')) {
            ...
            if (statementPath.isExportDeclaration()) {
                ...
                if (statementPath.isExportDefaultDeclaration()) {
                    statementPath.replaceWith(t.callExpression(implicitProvideIdentifier, [statementPath.node.declaration]));
                } else {
                    const specifiers = statementPath.node.specifiers;
