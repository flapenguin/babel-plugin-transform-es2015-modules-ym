...
const esModulesToYm = require('./transform');

const generatorOpts = { quotes: 'single', compact: true };

function transpile(filename, source, extraPlugins = [], esModulesToYmOptions = {}) {
    return transform(source, {
        filenameRelative: filename,
        plugins: [
            [ esModulesToYm, Object.assign({ sourceDir: 'src/' }, esModulesToYmOptions) ],
            ...extraPlugins
        ],
...
describe('transform', () => {
    ...
    it('should allow to customize ym namespace', () => {
        const actual = transpile('src/root/main.js', `export default 0;`, [], { ymGlobal: 'ymaps' });
        const expected = adapt(actual.metadata, `
            ymaps.modules.define('root.main', [], function(PROVIDE) {
                PROVIDE(0);
            });`);
